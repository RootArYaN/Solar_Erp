...
export const createFinanceTransaction = (body: Record<string, unknown>): Promise<FinanceTransaction> => apiRequest('/finance/transactions', { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
export const transferFinancialAccounts = (body: Record<string, unknown>): Promise<FinanceTransaction[]> => apiRequest('/finance/transfers', { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
...
export const createBill = (body: Record<string, unknown>): Promise<Bill> => apiRequest('/finance/bills', { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
export const recordBillPayment = (id: string, body: Record<string, unknown>): Promise<Bill> => apiRequest(`/finance/bills/${id}/payments`, { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
export const getCompanyLoans = (): Promise<CompanyLoan[]> => apiRequest('/finance/company-loans')
export const createCompanyLoan = (body: Record<string, unknown>): Promise<CompanyLoan> => apiRequest('/finance/company-loans', { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
export const recordCompanyLoanPayment = (id: string, body: Record<string, unknown>): Promise<CompanyLoan> => apiRequest(`/finance/company-loans/${id}/payments`, { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
