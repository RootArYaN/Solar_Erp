...
export const createInventoryItem = (body: Record<string, unknown>): Promise<InventoryItem> => apiRequest('/inventory/items', { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
export const createInventoryMovement = (body: Record<string, unknown>): Promise<InventoryMovement> => apiRequest('/inventory/movements', { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
...
export const createPoster = (body: Record<string, unknown>): Promise<Poster> => apiRequest('/posters', { method: 'POST', body, idempotencyKey: crypto.randomUUID() })
