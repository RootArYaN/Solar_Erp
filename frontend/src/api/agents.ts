...
export const createAgentTransaction = (token: string, id: string, input: CreateAgentTransactionInput): Promise<AgentTransaction> => apiRequest(`/agents/${id}/transactions`, { method: 'POST', token, body: input, idempotencyKey: crypto.randomUUID() })
export const createAgentCustomer = (token: string, id: string, input: CreateAgentCustomerInput): Promise<AgentOverview> => apiRequest(`/agents/${id}/customers`, { method: 'POST', token, body: input, idempotencyKey: crypto.randomUUID() })
