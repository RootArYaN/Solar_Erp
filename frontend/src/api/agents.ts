...
export const getAgents = (token: string): Promise<AgentListItem[]> => apiRequest('/agents', { token, cache: 'no-store' })
export const getAgentOverview = (token: string, id: string): Promise<AgentOverview> => apiRequest(`/agents/${id}/overview`, { token, cache: 'no-store' })
...
export const updateAgentTransaction = (token: string, id: string, transactionId: string, input: CreateAgentTransactionInput): Promise<AgentTransaction> => apiRequest(`/agents/${id}/transactions/${transactionId}`, { method: 'PATCH', token, body: input })
export const deleteAgentTransaction = (token: string, id: string, transactionId: string): Promise<void> => apiRequest(`/agents/${id}/transactions/${transactionId}`, { method: 'DELETE', token })
