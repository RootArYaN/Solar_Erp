...
export function companyHeader(template: DocumentPackTemplate, title: string, input: DocumentPackInput) {
  return `<header class="pack-doc__head"><div><strong>${esc(template.company_name || template.brand_name || 'Company')}</strong><span>${esc(template.address)}</span><small>${esc([template.phone, template.email, template.gstin && `GSTIN ${template.gstin}`].filter(Boolean).join(' · '))}</small></div><div><h3>${esc(title)}</h3><span>${esc(input.projectNumber || input.customerNumber)}</span><small>${new Date().toLocaleDateString('en-IN')}</small></div></header>`
}
...
export function signature(input: DocumentPackInput, template: DocumentPackTemplate) {
