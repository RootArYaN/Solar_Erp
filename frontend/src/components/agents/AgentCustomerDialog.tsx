...
export function AgentCustomerDialog({ busy, customer, onClose, onSubmit }: { busy: boolean; customer?: AgentCustomer; onClose: () => void; onSubmit: (value: CreateAgentCustomerInput) => Promise<void> }) {
  ...
  return <Modal title={customer ? 'Edit customer' : 'Register customer'} subtitle="Direct B2C customer and installation details" onClose={onClose}>
    <form className="admin-form" onSubmit={submit}>
      ...
      <div className="admin-form__grid"><label className="field"><span>District</span><input value={form.district} onChange={(event) => set('district', event.target.value)} /></label><label className="field"><span>Postal code</span><input value={form.postal_code} onChange={(event) => set('postal_code', event.target.value)} /></label></div>
      <div className="admin-form__grid"><label className="field"><span>Consumer number</span><input value={form.consumer_number} onChange={(event) => set('consumer_number', event.target.value)} /></label><label className="field"><span>Electricity provider</span><div className="field__control"><Zap size={17} /><input value={form.electricity_provider} onChange={(event) => set('electricity_provider', event.target.value)} /></div></label></div>
      <div className="admin-form__grid"><label className="field"><span>Lead source</span><input value={form.lead_source} onChange={(event) => set('lead_source', event.target.value)} /></label><label className="field"><span>Initial project / enquiry</span><input value={form.project_name} onChange={(event) => set('project_name', event.target.value)} placeholder="Example: 3.24 kW rooftop solar" /></label></div>
