import { Archive, Download, FileCheck2, FileText, LoaderCircle, RefreshCw, Save, Settings2, Upload } from 'lucide-react'
...
export function CustomerDataUploadPage({ session }: { session: Session }) {
  ...
  async function archive(file: StoredFile) {
    try { await setStoredFileStatus(file.id, 'archived'); if (snapshot) await loadCustomer(snapshot.customer.id); toast({ message: 'Document archived', variant: 'success' }) }
    catch (reason) { toast({ message: reason instanceof Error ? reason.message : 'Could not archive document', variant: 'error' }) }
  }
  ...
  if (loading && !customers.length) return <section className="erp-page"><LoadingSkeleton rows={7} /></section>
  if (error && !customers.length) return <section className="erp-page"><ErrorState message={error} onRetry={() => void load()} /></section>
  ...
  return <section className="erp-page">
    ...
    {!snapshot ? <EmptyState title="Select a customer" message="The document checklist and customer data pack will appear here." /> : <>
      <div className="erp-kpi-grid"><article><FileText /><span>Customer</span><strong>{customer?.display_name}</strong><small>{customer?.customer_type} · {customer?.electricity_provider || 'DISCOM not set'}</small></article><article><FileCheck2 /><span>Documents</span><strong>{files.length}</strong><small>Active uploaded files</small></article><article><FileText /><span>Project</span><strong>{project?.record_number || 'Not created'}</strong><small>{project ? `${project.capacity_kw} kW · ${project.payment_mode || 'Payment mode pending'}` : 'Quotation approval creates project'}</small></article><article><FileText /><span>Status</span><strong>{project?.documentation_status.replaceAll('_', ' ') || 'Customer registered'}</strong><small>{project?.registration_status.replaceAll('_', ' ') || 'Registration pending'}</small></article></div>

      <div className="erp-two-column document-workspace-grid"><section className="erp-panel"><header><div><span>Auto-filled information</span><h2>Customer data pack</h2></div></header><dl className="erp-detail-grid"><div><dt>Customer name</dt><dd>{customer?.display_name}</dd></div><div><dt>Phone</dt><dd>{customer?.contacts[0]?.phone || '—'}</dd></div><div><dt>Consumer number</dt><dd>{customer?.consumer_number || '—'}</dd></div><div><dt>Electricity provider</dt><dd>{customer?.electricity_provider || '—'}</dd></div><div className="erp-detail-wide"><dt>Site address</dt><dd>{customer?.site_address || customer?.addresses[0]?.line_1 || '—'}</dd></div><div><dt>System capacity</dt><dd>{project ? `${project.capacity_kw} kW` : '—'}</dd></div><div><dt>Approved value</dt><dd>{project?.approved_value || '—'}</dd></div><div><dt>Payment mode</dt><dd>{project?.payment_mode || 'Pending'}</dd></div><div><dt>Assigned agent</dt><dd>{customer?.assigned_agent_id ? 'Assigned' : 'Not assigned'}</dd></div></dl><div className="document-template-preview"><strong>{templateSettings.company_name || session.company.name}</strong><span>{templateSettings.address || 'Company address not configured'}</span><small>{templateSettings.gstin ? `GSTIN ${templateSettings.gstin}` : 'GSTIN not configured'}</small></div></section>

      <section className="erp-panel"><header><div><span>Required files</span><h2>Document checklist</h2></div></header><div className="document-checklist">{documentTypes.map(([key, label]) => { const match = files.find((file) => file.name.toLowerCase().includes(key.replaceAll('_', ' ')) || file.owner_type.includes(key)); return <article key={key}><span className={`document-check ${match ? 'is-done' : ''}`}><FileCheck2 size={15} /></span><div><strong>{label}</strong><small>{match ? `Uploaded ${shortDate.format(new Date(match.created_at))}` : 'Pending upload'}</small></div></article> })}</div></section></div>

      <section className="erp-panel"><header><div><span>Stored securely</span><h2>Uploaded documents</h2></div></header>{files.length ? <div className="erp-table-wrap"><table className="erp-table"><thead><tr><th>File</th><th>Type</th><th>Project</th><th>Uploaded</th><th>Status</th><th /></tr></thead><tbody>{files.map((file) => <tr key={file.id}><td><strong>{file.name}</strong><small>{Math.max(1, Math.round(file.size_bytes / 1024))} KB</small></td><td>{file.mime_type}</td><td>{file.project_id ? project?.record_number || 'Linked' : 'Customer level'}</td><td>{shortDate.format(new Date(file.created_at))}</td><td><span className="soft-badge soft-badge--success">{file.status}</span></td><td><div className="erp-row-actions"><button className="secondary-button secondary-button--compact" onClick={() => void downloadStoredFile(file.id, file.name)}><Download size={14} /> Download</button>{access.canArchive && <button className="icon-button" onClick={() => void archive(file)} aria-label="Archive file"><Archive size={14} /></button>}</div></td></tr>)}</tbody></table></div> : <EmptyState title="No documents uploaded" message="Upload the first customer document." />}</section>
    </>}
