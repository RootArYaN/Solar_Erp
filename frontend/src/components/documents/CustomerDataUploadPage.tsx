...
export function CustomerDataUploadPage({ session }: { session: Session }) {
  ...
  if (loading && !customers.length) return <WorkspacePage className="erp-page"><LoadingSkeleton rows={7} /></WorkspacePage>
  if (error && !customers.length) return <WorkspacePage className="erp-page"><ErrorState message={error} onRetry={() => void load()} /></WorkspacePage>
  ...
  return <WorkspacePage className="erp-page">
    ...
    {!snapshot ? <ScrollSurface className="document-empty-surface"><EmptyState title="Select a customer" message="The document checklist and customer data pack will appear here." /></ScrollSurface> : <>
      ...
      <ScrollSurface className="document-scroll-body">
