...
export function SolarPricingPage({ session }: { session: Session }) {
  ...
  async function save() {
    ...
    if (invalid) { toast({ message: 'Enter a name for every price item', variant: 'warning' }); return }
    setWorking(true)
    try { setBook(await savePricingBook({ name: book.name, items: book.items.map((row, index) => ({ ...row, display_order: index })) })); toast({ message: 'Pricing saved', variant: 'success' }) }
  ...
  return <WorkspacePage className="erp-page">
    <WorkspaceHeader className="erp-page-head"><div><span>Pricing</span><h1>Solar pricing</h1></div><div className="erp-head-actions"><button className="secondary-button" onClick={() => void load()}><RefreshCw size={15} /> Refresh</button>{access.canEdit && <button className="primary-button" onClick={() => void save()} disabled={working}><Save size={15} /> Save pricing</button>}</div></WorkspaceHeader>

    <KpiGrid columns={3} phoneColumns={1} responsive className="erp-kpi-grid"><article><IndianRupee /><span>Package total</span><strong>{money.format(total)}</strong><small>Active items with tax</small></article><article><Calculator /><span>Version</span><strong>v{book.version}</strong><small>Updated {new Intl.DateTimeFormat('en-IN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(book.updated_at))}</small></article><article><Plus /><span>Active items</span><strong>{book.items.filter((row) => row.is_active).length}</strong><small>{book.items.length} items in this price list</small></article></KpiGrid>

    <section className="erp-panel pricing-workspace-panel"><header className="pricing-book-head"><label><span>Price list name</span><input value={book.name} disabled={!access.canEdit} onChange={(event) => setBook({ ...book, name: event.target.value })} /></label><div className="pricing-book-actions"><div className="erp-search pricing-row-search"><Search size={14} /><input type="search" value={rowSearch} onChange={(event) => setRowSearch(event.target.value)} placeholder="Search price items" aria-label="Search price items" /></div>{access.canEdit && <button className="secondary-button" onClick={() => setBook({ ...book, items: [...book.items, emptyItem()] })}><Plus size={15} /> Add item</button>}</div></header>
