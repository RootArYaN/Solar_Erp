...
export function Bills({ data, billType, setBillType, dateFrom, setDateFrom, dateTo, setDateTo, reload, canEdit, onAdd, onEdit, onPay, onDelete, onUpload, onDownload, onRemoveAttachment, onDownloadMerged, downloadingMerged }: { data: BillList | null; billType: string; setBillType: (value: string) => void; reload: () => void; canEdit: boolean; onAdd: () => void; onEdit: (bill: Bill) => void; onPay: (bill: Bill) => void; onDelete: (bill: Bill) => void; onUpload: (bill: Bill, file: File) => void; onDownload: (bill: Bill) => void; onRemoveAttachment: (bill: Bill) => void; onDownloadMerged: () => void; downloadingMerged: boolean } & DateRangeProps) {
  return <>
    ...
    <BillTable rows={data?.data ?? []} onEdit={canEdit ? onEdit : undefined} onPay={canEdit ? onPay : undefined} onDelete={canEdit ? onDelete : undefined} onUpload={canEdit ? onUpload : undefined} onDownload={onDownload} onRemoveAttachment={canEdit ? onRemoveAttachment : undefined} />
  </>
}
...
export function BillTable({ rows, compact = false, onEdit, onPay, onDelete, onUpload, onDownload, onRemoveAttachment }: { rows: Bill[]; compact?: boolean; onEdit?: (bill: Bill) => void; onPay?: (bill: Bill) => void; onDelete?: (bill: Bill) => void; onUpload?: (bill: Bill, file: File) => void; onDownload?: (bill: Bill) => void; onRemoveAttachment?: (bill: Bill) => void }) {
  ...
  return <div className={`erp-table-wrap ${compact ? 'erp-table-wrap--compact' : ''}`}><table className="erp-table finance-bills-table"><thead><tr><th>Bill / party</th><th>Type</th><th>Total</th><th>Paid</th><th>Balance</th>{!compact && <th>Due</th>}<th>Status</th>{hasActions && <th />}</tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><strong>{row.bill_number}</strong><small>{row.customer_name || row.supplier_name}</small></td><td>{label(row.bill_type)}</td><td>{money.format(row.total_amount)}</td><td>{money.format(row.paid_amount)}</td><td>{money.format(row.balance_amount)}</td>{!compact && <td>{row.due_date ? shortDate.format(new Date(row.due_date)) : '—'}</td>}<td><span className="soft-badge">{label(row.payment_status)}</span></td>{hasActions && <td><div className="erp-row-actions finance-bill-row-actions">{onEdit && <button className="secondary-button secondary-button--compact" onClick={() => onEdit(row)}><Pencil size={13} /> Edit</button>}{onPay && row.balance_amount > 0 && <button className="secondary-button secondary-button--compact" onClick={() => onPay(row)}>Pay</button>}{onUpload && <label className="secondary-button secondary-button--compact finance-bill-upload finance-bill-document-action" title={`${row.file_id ? 'Replace' : 'Upload'} document for ${row.bill_number}`} aria-label={`${row.file_id ? 'Replace' : 'Upload'} document for ${row.bill_number}`}><Upload size={15} aria-hidden="true" /><input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" aria-label={`${row.file_id ? 'Replace' : 'Upload'} document for ${row.bill_number}`} onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ''; if (file) onUpload(row, file) }} /></label>}{onDownload && <button type="button" className="secondary-button secondary-button--compact finance-bill-document-action" disabled={!row.file_id} onClick={() => onDownload(row)} aria-label={row.file_id ? `Download document for ${row.bill_number}` : `No document uploaded for ${row.bill_number}`} title={row.file_id ? `Download document for ${row.bill_number}` : 'No document uploaded'}><Download size={15} aria-hidden="true" /></button>}{onRemoveAttachment && <button type="button" className="secondary-button secondary-button--compact finance-bill-document-action finance-bill-document-remove" disabled={!row.file_id} onClick={() => onRemoveAttachment(row)} aria-label={row.file_id ? `Remove document from ${row.bill_number}` : `No document uploaded for ${row.bill_number}`} title={row.file_id ? `Remove document from ${row.bill_number}` : 'No document uploaded'}><X size={15} aria-hidden="true" /></button>}{onDelete && <button type="button" className="danger-icon-button finance-transaction-delete" onClick={() => onDelete(row)} aria-label={`Delete bill ${row.bill_number}`} title="Delete bill"><Trash2 size={14} /></button>}</div></td>}</tr>)}</tbody></table></div>
}
