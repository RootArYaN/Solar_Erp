      ...
      <tbody>{rows.map((row) => <Fragment key={row.id}>
        <tr>
          ...
          {hasActions && <td><div className="erp-row-actions finance-bill-row-actions">
            {onEdit && <button className="secondary-button secondary-button--compact" onClick={() => onEdit(row)}><Pencil size={13} /> Edit</button>}
            {onPay && row.balance_amount > 0 && <button className="secondary-button secondary-button--compact" onClick={() => onPay(row)}>Pay</button>}
          ...
          <td colSpan={columnCount}>
            <div className="finance-bill-payments">
              ...
              {row.payments.map((payment) => <div className="finance-bill-payment" key={payment.id}>
                ...
                {onRemovePayment && <button type="button" onClick={() => onRemovePayment(row, payment)} aria-label={`Remove payment ${money.format(payment.amount)} from ${row.bill_number}`} title="Remove incorrect bill payment"><Trash2 size={14} /></button>}
              </div>)}
            </div>
          </td>
        </tr>}
