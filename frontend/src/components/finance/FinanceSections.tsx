...
import type { Bill, BillList, BillPayment, CompanyLoan, FinanceOverview, FinanceTransaction, FinanceTransactionList, FinancialAccount, Profitability } from '../../erp-types'
...
export function Overview({ data, dateFrom, setDateFrom, dateTo, setDateTo, reload }: { data: FinanceOverview; reload: () => void } & DateRangeProps) {
  const cards = [
    ...
    ['Customer payments due', data.customer_receivables, BadgeIndianRupee, 'warning'], ['Supplier payments due', data.supplier_payables, ReceiptText, 'warning'],
...
export function Transactions({ data, rows, search, setSearch, direction, setDirection, dateFrom, setDateFrom, dateTo, setDateTo, reload, reportMode, canEdit, onEdit, onReverse, onDelete }: { data: FinanceTransactionList | null; rows: FinanceTransactionList['data']; search: string; setSearch: (value: string) => void; direction: string; setDirection: (value: string) => void; dateFrom: string; setDateFrom: (value: string) => void; dateTo: string; setDateTo: (value: string) => void; reload: () => void; reportMode: boolean; canEdit: boolean; onEdit: (row: FinanceTransaction) => void; onReverse?: (row: FinanceTransaction) => void; onDelete?: (row: FinanceTransaction) => void }) {
  return <>
    ...
    <div className="finance-ledger-toolbar">
      ...
      <label className="finance-ledger-search"><Search size={13} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search transactions" aria-label="Search transactions" /></label>
...
export function Expenses({ data, dateFrom, setDateFrom, dateTo, setDateTo, reload, canEdit, onAdd, onEdit, onReverse, onDelete }: { data: FinanceTransactionList | null; dateFrom: string; setDateFrom: (value: string) => void; dateTo: string; setDateTo: (value: string) => void; reload: () => void; canEdit: boolean; onAdd: () => void; onEdit: (row: FinanceTransaction) => void; onReverse?: (row: FinanceTransaction) => void; onDelete?: (row: FinanceTransaction) => void }) {
  const isToday = dateFrom === today() && dateTo === today()
  return <>
    <section className="mini-kpis finance-expense-kpis">
      <article><span>{isToday ? "Today's expense" : 'Selected expenses'}</span><strong>{money.format((data?.money_out ?? 0) - (data?.money_in ?? 0))}</strong></article>
      <article><span>Entries</span><strong>{data?.total ?? 0}</strong></article>
      <article><span>Average</span><strong>{money.format(((data?.money_out ?? 0) - (data?.money_in ?? 0)) / Math.max(1, data?.total ?? 0))}</strong></article>
    </section>
    ...
    <TransactionTable rows={data?.data ?? []} canEdit={canEdit} onEdit={onEdit} onReverse={onReverse} onDelete={onDelete} />
  </>
}
...
export function Bills({ data, billType, setBillType, dateFrom, setDateFrom, dateTo, setDateTo, reload, canEdit, onAdd, onEdit, onPay, onDelete, onUpload, onDownload, onRemoveAttachment, onRemovePayment, onDownloadMerged, downloadingMerged }: { data: BillList | null; billType: string; setBillType: (value: string) => void; reload: () => void; canEdit: boolean; onAdd: () => void; onEdit: (bill: Bill) => void; onPay: (bill: Bill) => void; onDelete?: (bill: Bill) => void; onUpload: (bill: Bill, file: File) => void; onDownload: (bill: Bill) => void; onRemoveAttachment: (bill: Bill) => void; onRemovePayment?: (bill: Bill, payment: BillPayment) => void; onDownloadMerged: () => void; downloadingMerged: boolean } & DateRangeProps) {
  return <>
    <section className="mini-kpis finance-bill-kpis">
      ...
      <article><span>Amount due</span><strong>{money.format(data?.data.reduce((sum, row) => sum + row.balance_amount, 0) ?? 0)}</strong></article>
    ...
    <div className="finance-bills-toolbar">
      ...
      <button className="secondary-button finance-bill-download-all" onClick={onDownloadMerged} disabled={downloadingMerged}><Download size={14} /> {downloadingMerged ? 'Merging…' : 'Download PDF'}</button>
      {canEdit && <button className="primary-button finance-bill-create" onClick={onAdd}><Plus size={14} /> Create bill</button>}
    </div>
    <BillTable rows={data?.data ?? []} onEdit={canEdit ? onEdit : undefined} onPay={canEdit ? onPay : undefined} onDelete={canEdit ? onDelete : undefined} onUpload={canEdit ? onUpload : undefined} onDownload={onDownload} onRemoveAttachment={canEdit ? onRemoveAttachment : undefined} onRemovePayment={canEdit ? onRemovePayment : undefined} />
  </>
}
...
export function TransactionTable({ rows, compact = false, canEdit = false, onEdit, onReverse, onDelete }: { rows: FinanceTransactionList['data']; compact?: boolean; canEdit?: boolean; onEdit?: (row: FinanceTransaction) => void; onReverse?: (row: FinanceTransaction) => void; onDelete?: (row: FinanceTransaction) => void }) {
  ...
  return <div className={`erp-table-wrap ${compact ? 'erp-table-wrap--compact' : ''}`}><table className="erp-table"><thead><tr><th>Date</th><th>Transaction / party</th><th>Category / source</th>{!compact && <th>Account</th>}<th>Money in</th><th>Money out</th>{!compact && <th>Status</th>}{canEdit && <th />}</tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td>{shortDate.format(new Date(row.transaction_date))}</td><td><strong>{row.transaction_number}</strong><small>{row.party_name || row.description}</small></td><td>{row.category_name || label(row.source_type)}<small>{row.reference_number}</small></td>{!compact && <td>{row.account_name}<small>{label(row.payment_method)}</small></td>}<td className="money-in">{row.direction === 'credit' ? money.format(row.amount) : '—'}</td><td className="money-out">{row.direction === 'debit' ? money.format(row.amount) : '—'}</td>{!compact && <td><span className="soft-badge">{label(row.status)}</span></td>}{canEdit && <td><div className="erp-row-actions">{row.status === 'posted' && onEdit && <button className="secondary-button secondary-button--compact" onClick={() => onEdit(row)}><Pencil size={13} /></button>}{row.status === 'posted' && onReverse && <button className="secondary-button secondary-button--compact" onClick={() => onReverse(row)}><RotateCcw size={13} /></button>}{onDelete && <button type="button" className="danger-icon-button finance-transaction-delete" onClick={() => onDelete(row)} aria-label={`Delete ${row.transaction_number}`} title="Delete transaction"><Trash2 size={14} /></button>}</div></td>}</tr>)}</tbody></table></div>
}

export function BillTable({ rows, compact = false, onEdit, onPay, onDelete, onUpload, onDownload, onRemoveAttachment, onRemovePayment }: { rows: Bill[]; compact?: boolean; onEdit?: (bill: Bill) => void; onPay?: (bill: Bill) => void; onDelete?: (bill: Bill) => void; onUpload?: (bill: Bill, file: File) => void; onDownload?: (bill: Bill) => void; onRemoveAttachment?: (bill: Bill) => void; onRemovePayment?: (bill: Bill, payment: BillPayment) => void }) {
  if (!rows.length) return <EmptyState title="No bills found" />
  const hasActions = Boolean(onEdit || onPay || onDelete || onUpload || onDownload || onRemoveAttachment)
  const columnCount = 6 + (compact ? 0 : 1) + (hasActions ? 1 : 0)
  return <div className={`erp-table-wrap ${compact ? 'erp-table-wrap--compact' : ''}`}>
    <table className="erp-table finance-bills-table">
      <thead><tr><th>Bill / party</th><th>Type</th><th>Total</th><th>Paid</th><th>Balance</th>{!compact && <th>Due</th>}<th>Status</th>{hasActions && <th />}</tr></thead>
      <tbody>{rows.map((row) => <Fragment key={row.id}>
        <tr>
          ...
          {hasActions && <td><div className="erp-row-actions finance-bill-row-actions">
            {onEdit && row.status !== 'cancelled' && <button className="secondary-button secondary-button--compact" onClick={() => onEdit(row)}><Pencil size={13} /></button>}
            {onPay && row.status !== 'cancelled' && row.balance_amount > 0 && <button className="secondary-button secondary-button--compact" onClick={() => onPay(row)}>Pay</button>}
            {onUpload && <label className="secondary-button secondary-button--compact finance-bill-upload finance-bill-document-action" title={`${row.file_id ? 'Replace' : 'Upload'} document for ${row.bill_number}`} aria-label={`${row.file_id ? 'Replace' : 'Upload'} document for ${row.bill_number}`}><Upload size={15} aria-hidden="true" /><input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" aria-label={`${row.file_id ? 'Replace' : 'Upload'} document for ${row.bill_number}`} onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ''; if (file) onUpload(row, file) }} /></label>}
            {onDownload && <button type="button" className="secondary-button secondary-button--compact finance-bill-document-action" disabled={!row.file_id} onClick={() => onDownload(row)} aria-label={row.file_id ? `Download document for ${row.bill_number}` : `No document uploaded for ${row.bill_number}`} title={row.file_id ? `Download document for ${row.bill_number}` : 'No document uploaded'}><Download size={15} aria-hidden="true" /></button>}
            {onRemoveAttachment && <button type="button" className="secondary-button secondary-button--compact finance-bill-document-action finance-bill-document-remove" disabled={!row.file_id} onClick={() => onRemoveAttachment(row)} aria-label={row.file_id ? `Remove document from ${row.bill_number}` : `No document uploaded for ${row.bill_number}`} title={row.file_id ? `Remove document from ${row.bill_number}` : 'No document uploaded'}><X size={15} aria-hidden="true" /></button>}
            {onDelete && <button type="button" className="danger-icon-button finance-transaction-delete" onClick={() => onDelete(row)} aria-label={`Void bill ${row.bill_number}`} title="Void bill"><RotateCcw size={14} /></button>}
          </div></td>}
        </tr>
        {!compact && (row.payments?.length ?? 0) > 0 && <tr className="finance-bill-payments-row">
          <td colSpan={columnCount}>
            <div className="finance-bill-payments">
              ...
              {row.payments.map((payment) => <div className="finance-bill-payment" key={payment.id}>
                <span><CalendarDays size={13} /> {shortDate.format(new Date(payment.transaction_date))}</span>
                <strong className={row.bill_type === 'sales' ? 'money-in' : 'money-out'}>{row.bill_type === 'sales' ? 'Received' : 'Paid'} {money.format(payment.amount)}</strong>
                <span>{payment.account_name} · {label(payment.payment_method)}</span>
                <span>{payment.transaction_number}{payment.reference_number ? ` · ${payment.reference_number}` : ''}</span>
                {onRemovePayment && payment.status === 'posted' && <button type="button" onClick={() => onRemovePayment(row, payment)} aria-label={`Reverse payment ${money.format(payment.amount)} from ${row.bill_number}`} title="Reverse incorrect bill payment"><RotateCcw size={14} /></button>}
              </div>)}
            </div>
          </td>
        </tr>}
