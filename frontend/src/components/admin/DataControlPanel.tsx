    ...
    <section className="data-panel">
      ...
      {loading ? <LoadingSkeleton rows={6} /> : tab === 'deleted' ? (
        ...
        events.length ? <div className="user-table-wrap"><table className="user-table"><thead><tr><th>Event</th><th>Entity</th><th>Actor</th><th>Date</th></tr></thead><tbody>
          {events.map((row) => <tr key={row.id}><td><strong>{title(row.event)}</strong><small>{row.request_id || 'No request ID'}</small></td><td>{title(row.entity)}<small>{row.entity_id}</small></td><td><strong>{row.actor_name || 'System'}</strong><small>{title(row.user_role)}</small></td><td>{dateTime.format(new Date(row.created_at))}</td></tr>)}
        ...
        <div className="user-table-wrap"><table className="user-table"><thead><tr><th>Movement</th><th>Item</th><th>Quantity</th><th>Reason</th><th>Actor</th><th>Date</th></tr></thead><tbody>
          {movements.map((row) => <tr key={row.id}><td><strong>{row.reference_number}</strong><small>{title(row.movement_type)} · {title(row.status)}</small></td><td>{row.item_name}</td><td>{row.corrected_quantity != null ? `${row.quantity} → ${row.corrected_quantity}` : row.quantity}</td><td>{row.reason || '—'}</td><td><strong>{row.actor_name || 'System'}</strong></td><td>{dateTime.format(new Date(row.created_at))}</td></tr>)}
        </tbody></table></div>
