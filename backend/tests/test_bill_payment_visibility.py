...
def test_deleting_a_finalized_bill_payment_requires_reversal(monkeypatch):
    bill = SimpleNamespace(id='bill-1', company_id='company-1')
    monkeypatch.setattr(finance_service, '_load_visible_bill', lambda _db, _actor, _bill_id: bill)
    actor = SimpleNamespace(membership=SimpleNamespace(company_id='company-1'))

    with pytest.raises(finance_service.FinanceConflictError, match='Reverse the payment'):
        finance_service.delete_bill_payment(SimpleNamespace(), actor, 'bill-1', 'payment-1')


def test_reversing_a_bill_payment_uses_its_linked_transaction(monkeypatch):
    ...
    monkeypatch.setattr(finance_service, '_load_visible_bill', lambda _db, _actor, _bill_id, for_update=False: bill)
    monkeypatch.setattr(finance_service, 'reverse_transaction', lambda _db, _actor, transaction_id, _payload, commit: reversed_ids.append(transaction_id))
    monkeypatch.setattr(finance_service, '_bill_summary', lambda _db, row: {'id': row.id})
    actor = SimpleNamespace(membership=SimpleNamespace(company_id='company-1'))
    payload = finance_service.ReverseFinanceTransactionRequest(
        transaction_date=date(2026, 8, 6), reason='Correct payment entry',
    )

    result = finance_service.reverse_bill_payment(db, actor, 'bill-1', 'payment-1', payload)

    assert reversed_ids == ['transaction-1']
